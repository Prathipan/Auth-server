...
router.post("/login" , async (req,res,next) => {
  try{
    ...
    const user =await db.collection("register").findOne({email : req.body.email});
...
router.post('/register', async(req, res, next) =>  {
  try{
    ...
    if(!present){
       const user = await db.collection("register").insertOne({name : name , email : email , password : hash , cpassword : hash1})
...
router.get("/validUser", authenticate ,async(req,res,) => {
  try{
     ...
     const user = await db.collection("register").findOne({_id : mongodb.ObjectId(req.id)});
...
router.post("/sendpasswordlink", async(req,res) => {
    ...
    try{
      ...
      const setUser =await db.collection("register").findOneAndUpdate({_id : mongodb.ObjectId(user._id)},{$set:{verifyToken : token}})
...
router.get("/resetpassword/:id/:token", async(req,res) => {
    ...
    try{
       const db = await connectDB();
       const validUser = db.collection("register").findOne({_id : mongodb.ObjectId(id)});
...
router.post("/:id/:token",async(req,res) => {
  ...
  try{
    ...
    if(validUser && verifyToken._id){
      ...
      const setNewPassword = await db.collection("register").findOneAndUpdate({_id:mongodb.ObjectId(id)},{$set : {password : hash,cpassword : hash1}})
